...
class ReceiptPrinterTest(unittest.TestCase):
    ...
    def test_one_line_item(self):
        self.receipt.add_product(self.toothbrush, 1, 0.99)
        verify(ReceiptPrinter().print_receipt(self.receipt))

    def test_quantity_two(self):
        self.receipt.add_product(self.toothbrush, 2, 0.99)
        verify(ReceiptPrinter().print_receipt(self.receipt))

    def test_loose_weight(self):
        self.receipt.add_product(self.apples, 2.3, 1.99)
        verify(ReceiptPrinter().print_receipt(self.receipt))

    def test_total(self):
        self.receipt.add_product(self.toothbrush, 2, 0.99)
        self.receipt.add_product(self.apples, 0.75, 1.99)
        verify(ReceiptPrinter().print_receipt(self.receipt))
